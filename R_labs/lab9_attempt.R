################# Lab9.1: Support Vector Classifier ########## ------------------------------------------------------------------------------------
...
## Check whether the classes are linarly separable
plot(x, col = (3 + y), pch = 19)
## They are not!

#X were generated using the same random distribution. To make points seperable, we move some points by 1
x[y==1,] = x[y==1,] + 1 #x[y==1,] selects all x values where y==1 is true (aka the last 10 values)
...
## A cost argument allow us to specifiy the cost of a violation to the margin. 
...
## The plot looks a little bit jagged here, not too much control on the color, and puts x1 and x2 on a non-conventional way
...
## We can perform cross validation using tune() to select the best choice of gamma and cost for a SVM with a radial kernel
set.seed(1)
...
########### Lab 9.3: Application to Gene Expression Data ######################## -------------------------------------------------------------------
library(ISLR)
data(Khan)
## This data consists of a number of tissue samples corresponding to four distinct types of small round blue cell tumors
dim(Khan$xtrain)
dim(Khan$xtest)
...
## We should use a linear kernel (reduce the model flexibility), since the number of variables is much larger than the number of observations.
ans = svm(y~., data = dat, kernel = "linear", cost = 10)
summary(ans)
table(ans$fitted, dat$y)



## Let us investigate the performance on testing observations
dat.test = data.frame(x = Khan$xtest, y = as.factor(Khan$ytest))
pred.test = predict(ans, newdat = dat.test)
